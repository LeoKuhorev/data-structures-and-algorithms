...
const returnNames = (arr) =>
  arr.reduce((acc, character) => [...acc, character.name], []);
...
const reversedString = (str) =>
  str.split("").reduce((acc, letter) => letter + acc, "");
...
const countNumberOfChildren = (arr) =>
  arr.reduce(
    (sum, person) => (person.children ? (sum += person.children.length) : sum),
    0
  );
...
const countPrimeNumbers = (arr) =>
  arr.reduce((acc, val) => (isPrime(val) ? ++acc : acc), 0);
